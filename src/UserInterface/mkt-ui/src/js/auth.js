  ...
  async init(appInitCallback, errorCallback) {
    ...
    this._keycloak.init({ onLoad: "login-required"}).then((authenticated) => {
      if (authenticated) {
        ...
        console.log(this._keycloak.tokenParsed["resource_access"]["vue-demo"]["roles"].indexOf("AdminUser"));

        accountInfo.isAdmin = (this._keycloak.tokenParsed["resource_access"]["vue-demo"]["roles"].indexOf("AdminUser") != -1);
